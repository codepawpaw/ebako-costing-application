...
class model_costing extends CI_Model {
    ...
    function selectByIdAndCustomerId($id, $customer_id) {
        $query = "select 
                costing.*,
                model.no code,
                model.custcode,
                ...
                from costing
                left join model on costing.modelid=model.id
                left join customer on costing.customerid=customer.id where costing.id=$id AND customerid=$customer_id";
        return $this->db->query($query)->row();
    }
    ...
    function updatePropertyById($fob_price, $rate_value, $picklist_ratevalue, $costingid) {
        $this->db->update('costing', array("fob_price" => $fob_price, "ratevalue" => $rate_value, "picklist_ratevalue" => $picklist_ratevalue), array("id" => $costingid));
    }
